...
enum acer_wmi_event_ids {
	...
	WMID_GAMING_TURBO_KEY_EVENT_V2 = 0x8,
};
...
static const struct dmi_system_id acer_quirks[] __initconst = {
	...
	{
		.callback = dmi_matched,
		.ident = "Acer Predator PT51S (Aspier XXXX DMI)",
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR,"Acer"),
			DMI_MATCH(DMI_PRODUCT_NAME,"Aspier XXXX"),
		},
		.driver_data = &quirk_acer_predator_pt51s,
	},
	{
		.callback = dmi_matched,
		.ident = "Acer Predator PT51S",
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR,"Acer"),
			DMI_MATCH(DMI_PRODUCT_NAME,"Predator PT51S"),
		},
		.driver_data = &quirk_acer_predator_pt51s,
	},
	{
		.callback = dmi_matched,
		.ident = "Acer Predator Volante ADH",
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR,"Acer"),
			DMI_MATCH(DMI_BOARD_NAME,"Volante_ADH"),
		},
		.driver_data = &quirk_acer_predator_pt51s,
	},
	{
...
static void __init find_quirks(void)
{
	...
	if (quirks == NULL) {
		if (wmi_has_guid(WMID_GUID3) && wmi_has_guid(WMID_GUID4)) {
			pr_info("No DMI quirk matched; using generic Predator gaming WMI fallback\n");
			quirks = &quirk_acer_predator_v4;
		} else {
...
{
	int cpu_ret = -1, gpu_ret = -1;

	if (turbo_state) {
		turbo_state = 0;
		WMID_gaming_set_u64(0x1, ACER_CAP_TURBO_LED);
		if (has_cap(ACER_CAP_PWM)) {
			WMID_gaming_set_fan_behavior(ACER_GAMING_FAN_BEHAVIOR_CPU, ACER_WMID_FAN_MODE_AUTO);
			WMID_gaming_set_fan_behavior(ACER_GAMING_FAN_BEHAVIOR_GPU, ACER_WMID_FAN_MODE_AUTO);
		} else {
		...
		if (has_cap(ACER_CAP_PWM)) {
			WMID_gaming_set_fan_behavior(ACER_GAMING_FAN_BEHAVIOR_CPU, ACER_WMID_FAN_MODE_TURBO);
			WMID_gaming_set_fan_behavior(ACER_GAMING_FAN_BEHAVIOR_GPU, ACER_WMID_FAN_MODE_TURBO);
		} else {
	...
	, void *context)
{
	...
	case WMID_GAMING_TURBO_KEY_EVENT_V2:
		if (return_value.key_num == 0x1) {
			if(return_value.device_state >= 1 && return_value.device_state <= 3)
				...
				pr_info("turbo key event: key_num=0x1 device_state=0x%x, treating as turbo toggle\n",
			...
			pr_info("turbo key event: key_num=0x%x device_state=0x%x, treating as turbo toggle\n",
				...
				u32 attr, int channel, long val)
{
	...
	case hwmon_pwm:
		...
		case hwmon_pwm_input:
			...
			ret = WMID_gaming_set_gaming_fan_speed(fan, speed);
			if (ret < 0)
				dev_err(dev, "fan speed write failed ch=%d fan=%u speed=%u ret=%d\n",
					channel, fan, speed, ret);
			return ret;
		case hwmon_pwm_enable:
			fan_bitmap = acer_wmi_fan_channel_to_fan_bitmap[channel];
			switch (val) {
			...
			ret = WMID_gaming_set_fan_behavior(fan_bitmap, mode);
			if (ret < 0)
				dev_err(dev, "fan behavior write failed ch=%d bitmap=0x%x mode=%u ret=%d\n",
					channel, fan_bitmap, mode, ret);
			return ret;
		default:
			return -EOPNOTSUPP;
		}
...
static int __init acer_wmi_init(void)
{
    ...
    if (force_caps != -1) {
        pr_info("force_caps=0x%x OR-ing into capability 0x%x\n",
                force_caps, interface->capability);
